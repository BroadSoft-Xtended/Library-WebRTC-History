...
function History(stats, eventbus, sipstack, urlconfig) {
  ...
  self.bindings = {
    'classes': {
      history: ['visible', 'enableCallHistory'],
      sipstack: 'callState'
    },
    'content': {
  ...
  var createCall = function(rtcSession) {
    ...
    call.bitrateIn = stats.getAvg("video", "kiloBitsReceivedPerSecond") || stats.getAvg("audio", "kiloBitsReceivedPerSecond");
    call.bitrateOut = stats.getAvg("video", "kiloBitsSentPerSecond") || stats.getAvg("audio", "kiloBitsSentPerSecond");
  ...
  self.init = function() {
    self.enableCallHistory = urlconfig.enableCallHistory || self.enableCallHistory;
    self.pageNumber = 0;
    self.callsPerPage = 4;
    self.maxPages = 10;
  };

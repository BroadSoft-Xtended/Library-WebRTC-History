...
function History(stats, configuration, eventbus) {
  ...
  var updateClasses = function(){
    self.classes = [
      self.visible ? 'history-shown' : 'history-hidden',
      configuration.enableCallHistory ? 'enableCallHistory' : '',
      sipstack.callState      
    ];
  };
  ...
  self.props = {'isForwardEnabled': true, 'isBackEnabled': true, 'calls': true, 'classes': true, 'visible': {
    onSet: function(){
      updateClasses();
    }
  }};
  ...
  self.listeners = function(sipstackDatabinder, configurationDatabinder) {
    ...
    configurationDatabinder.onModelPropChange('enableCallHistory', function(){
      updateClasses();
    });
